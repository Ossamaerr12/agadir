...
// API Key الخاص بك
const apiKey = 'a8e539b8902b8bc1165ffa9a';

// وظيفة لتحميل أسعار الصرف مع اليورو كعملة أساسية
function loadExchangeRates(currency) {
    // الرابط مع اليورو كعملة أساسية
    const url = `https://v6.exchangerate-api.com/v6/${apiKey}/latest/EUR`;  // استخدام Backticks

    // استخدام Fetch API لجلب البيانات
    fetch(url)
        .then(response => response.json())
        .then(data => {
            // الحصول على سعر الصرف للعملة المحددة (مثل MAD أو USD)
            const exchangeRate = data.conversion_rates[currency];

            // تغيير الثمن في جميع البطاقات
            updatePricesInAllCards(exchangeRate, currency);
        })
        .catch(error => {
...
// تحديث الثمن في جميع البطاقات بناءً على العملة
function updatePricesInAllCards(exchangeRate, currency) {
    // تحديد جميع البطاقات التي تحتوي على class "car-card"
    const priceElements = document.querySelectorAll('.car-card .card-price');
    const priceInEUR = 200; // الثمن الأصلي باليورو

    // التكرار على كل عنصر وتحديث السعر
    priceElements.forEach(priceElement => {
        // حساب السعر بالعملة المحددة
        const priceInSelectedCurrency = (priceInEUR * exchangeRate).toFixed(2);

        // تحديث الثمن في العنصر مع العملة
        priceElement.textContent = `${priceInSelectedCurrency} ${currency}`; // استخدام Backticks بشكل صحيح
    });
}

// إضافة مستمع لحدث تغيير العملة
document.querySelectorAll('.dropdown-menu a').forEach(item => {
    item.addEventListener('click', function (e) {
        e.preventDefault();
        const selectedCurrency = e.target.getAttribute('data-currency');
        
        // تحميل الأسعار بناءً على العملة المحددة
